...
|-----------|--------|-------|
| **Type Safety** | ✅ Met | TypeScript strict mode; Zod for runtime validation |
| **Testing Standards** | ✅ Met | Unit (Jest) + contract + e2e (Playwright); > 70% coverage target |
| **UX Consistency** | ✅ Met | PWA first; mobile-friendly forms; < 10 min onboarding |
| **Performance** | ✅ Met | LCP < 2s target; Vite for fast HMR |
| **Security & Compliance** | ✅ Met | HTTPS, JWT, input validation; manual payment only (no PCI burden) |
| **Development Velocity** | ✅ Met | Monorepo with shared types; Fastify for fast iteration |
...
│   └── social-preview.schema.json  # Template schema for social images
...
├── backend/
│   ├── src/
│   │   ├── models/         # TypeORM entities (User, Business, Dish, etc.)
│   │   ├── services/       # Business logic (OrderService, MenuService, etc.)
│   │   ├── routes/         # Fastify route handlers (organized by domain)
│   │   ├── middleware/     # Auth, error handling, logging
│   │   ├── utils/          # Validation (Zod schemas), helpers
│   │   └── main.ts         # Fastify server entry
│   ├── tests/
│   │   ├── unit/           # Business logic & service tests (Jest)
│   │   ├── contract/       # API endpoint schema validation
│   │   └── integration/    # End-to-end workflows
│   ├── migrations/         # Database schema (TypeORM or Flyway)
│   ├── package.json
│   └── tsconfig.json
├── frontend/
│   ├── src/
│   │   ├── components/     # React components (forms, lists, cards)
│   │   ├── pages/          # Routes (Onboarding, Dashboard, PublicMenu, etc.)
│   │   ├── services/       # API client (fetch wrapper, retry logic)
│   │   ├── types/          # Shared TypeScript types (from backend)
│   │   ├── styles/         # TailwindCSS config, global styles
│   │   └── main.tsx        # React entry & root route
│   ├── tests/
│   │   ├── e2e/            # Playwright tests (critical flows)
│   │   └── unit/           # Component unit tests
│   ├── public/             # Static assets (favicon, manifest.json for PWA)
│   ├── vite.config.ts
│   ├── package.json
│   └── tsconfig.json
├── shared/                 # (Optional) Shared TypeScript types, validators
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Browser (PWA) — React + TailwindCSS + TypeScript                │
│ ├─ Onboarding flow (signup, login, business profile)            │
│ ├─ Seller Dashboard (menu editor, orders, reports)              │
│ ├─ Public Menu (shareable link for customers)                   │
│ └─ Order Form (customer checkout)                               │
└─────────────────┬───────────────────────────────────────────────┘
                  │ HTTPS (API calls)
                  ▼
┌─────────────────────────────────────────────────────────────────┐
...
│ ├─ /businesses/* (profile, settings, delivery rules)            │
│ ├─ /dishes/* (CRUD dishes)                                      │
│ ├─ /menus/* (CRUD menus, public menu endpoint)                  │
│ ├─ /orders/* (create order, view orders, update status)         │
│ ├─ /media/* (image upload, social preview generation)           │
│ ├─ /ocr/* (OCR parse endpoint — stub for Phase 2)              │
│ ├─ /reports/* (orders list, CSV export)                         │
│ └─ /webhooks/* (email notifications async queue)                │
│                                                                  │
...
**Core Endpoints** (MVP):
- `POST /auth/signup` — Seller email/password signup
- `POST /auth/login` — Email/password login (returns JWT)
- `GET /auth/me` — Current user info
- `POST /businesses` — Create business profile
- `GET /businesses/{id}` — Get business details
- `PUT /businesses/{id}` — Update business profile
- `GET /businesses/{id}/settings` — Delivery rules, payment info
- `PUT /businesses/{id}/settings` — Update settings
- `POST /dishes` — Create dish (auth required)
- `PUT /dishes/{id}` — Update dish
- `DELETE /dishes/{id}` — Delete dish
- `POST /menus` — Create menu (auth required)
- `GET /menus/{id}` — Get menu details (public, no auth)
- `PUT /menus/{id}` — Update menu (auth required)
- `GET /businesses/{id}/menu/current` — Get current active menu for business (public)
- `POST /orders` — Create order (public or auth)
- `GET /businesses/{id}/orders` — List orders (auth required)
- `PUT /orders/{id}` — Update order status/notes (auth required)
...
### Overview
MenuMaker will extend beyond web PWA with native mobile apps for iOS and Android starting Phase 3.5 (Month 12–18). This section documents the mobile technology strategy and integration with existing backend.
...
3. **Time to Market**: 8–10 weeks to launch both platforms simultaneously (vs. 16–20 weeks native)
...
- ✅ **Seller Features**: Onboarding, menu editor, order management, reporting, subscription tiers
- ✅ **Customer Features**: Marketplace discovery, menu browsing, checkout, order history, re-order
- ✅ **Payment Integration**: Same processors as web (Stripe, Razorpay, PhonePe)
- ✅ **Notifications**: Push notifications (FCM + APNs) for new orders, menu updates
- ✅ **Offline Support**: Orders cached locally, sync when online

#### Platform-Specific Features
- 🍎 **iOS**: Siri Shortcuts, App Clips, Home screen widgets, Apple Pay
- 🤖 **Android**: Google Assistant shortcuts, Material You theming, Wear OS, Android Auto (future)
...
menumaker-mobile/
├── src/
│   ├── screens/         # Seller + Customer UI screens (Seller Dashboard, Marketplace, etc.)
│   ├── components/      # Reusable components (MenuCard, OrderItem, etc.)
│   ├── services/        # API client, storage, camera (shared with web backend APIs)
│   ├── store/          # Zustand state management
│   └── App.tsx         # Navigation entry point
├── ios/                # Native Swift code for OCR, auth
├── android/            # Native Kotlin code for OCR, auth
├── tests/              # Jest unit + Detox E2E tests
...
|-------|----------|--------------|
| **Setup** | Week 1–2 | React Native project, CI/CD, design system |
| **Auth & Core** | Week 3–7 | Signup, seller dashboard, menu editor, customer marketplace |
| **Advanced** | Week 8–9 | Camera OCR, notifications, offline sync, widgets |
| **QA & Beta** | Week 10–11 | Performance testing, bug fixes, TestFlight/Play Store beta |
...
- ✅ 1,000+ downloads per platform by Month 2 post-launch
...
- ✅ Manual payment flow tested with 3+ sellers
- ✅ Docs & README updated for next dev
...
7. **Iterate**: Complete user stories 1–7 in priority order

---

**Ready for**: `/speckit.plan` validation → `/speckit.tasks` generation → `/speckit.implement`
